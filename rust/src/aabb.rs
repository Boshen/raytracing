...
impl AABB {
    pub fn new (mins: Vec<Vec3>, maxs: Vec<Vec3>) -> AABB {
        return AABB {
            min: [get_min(&mins, &|v: &Vec3| v.x),
                  get_min(&mins, &|v: &Vec3| v.y),
                  get_min(&mins, &|v: &Vec3| v.z)],
            max: [get_max(&maxs, &|v: &Vec3| v.x),
...
fn get_min(xs: &Vec<Vec3>, acc: &dyn Fn(&Vec3) -> f64) -> f64 {
    return xs.iter().map(|x| acc(&x)).fold(f64::INFINITY, |a, b| a.min(b))
}

fn get_max(xs: &Vec<Vec3>, acc: &dyn Fn(&Vec3) -> f64) -> f64 {
    return xs.iter().map(|x| acc(&x)).fold(-f64::INFINITY, |a, b| a.max(b))
}
